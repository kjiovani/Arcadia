...
if (file_exists($csrf)) require_once $csrf;
if (!function_exists('csrf_verify')) {
  function csrf_verify() {
    ...
    if (!$ok) { http_response_code(419); echo json_encode(['ok'=>false,'msg'=>'CSRF']); exit; }
  }
}
...
if (empty($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
  echo json_encode(['ok'=>false,'msg'=>'Tidak ada file / upload error']); exit;
}

$f   = $_FILES['file'];
$max = 3 * 1024 * 1024; // 3MB
if ($f['size'] > $max) { echo json_encode(['ok'=>false,'msg'=>'Maks 3MB']); exit; }

$mime = @mime_content_type($f['tmp_name']) ?: '';
$allow = ['image/png'=>'png','image/jpeg'=>'jpg','image/webp'=>'webp','image/gif'=>'gif','image/svg+xml'=>'svg'];
if (!isset($allow[$mime])) { echo json_encode(['ok'=>false,'msg'=>'Tipe gambar tidak didukung']); exit; }
$ext = $allow[$mime];

if ($ext==='svg') {
  $svg = file_get_contents($f['tmp_name']);
  if (stripos($svg,'<script')!==false) { echo json_encode(['ok'=>false,'msg'=>'SVG tidak valid']); exit; }
}
...
if (!move_uploaded_file($f['tmp_name'], $dest)) { echo json_encode(['ok'=>false,'msg'=>'Gagal simpan']); exit; }
