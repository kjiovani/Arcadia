...
if($action==='create' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    db_exec($mysqli,"INSERT INTO mediafiles(walk_id,file_type,file_url,caption) VALUES(?,?,?,?)",
      [$walk_id,$file_type,$file_url,$caption],'isss');
    flash('ok','Media ditambahkan.'); header('Location: mediafiles.php'); exit;
  } catch(Exception $e){ flash('err',$e->getMessage()); }
}
if($action==='delete'){
  $id=(int)($_GET['id']??0);
  db_exec($mysqli,"DELETE FROM mediafiles WHERE id=?",[$id],'i');
  flash('ok','Media dihapus.'); header('Location: mediafiles.php'); exit;
}
