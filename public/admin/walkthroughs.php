...
if($action==='create' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    db_exec($mysqli,"INSERT INTO walkthroughs(game_id,title,overview,difficulty) VALUES(?,?,?,?)",
      [$game_id,$title,$overview,$difficulty],'isss');
...
if($action==='update' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    $overview = str_trim($_POST['overview'] ?? '');
    $difficulty = str_trim($_POST['difficulty'] ?? 'Medium');
    db_exec($mysqli,"UPDATE walkthroughs SET game_id=?, title=?, overview=?, difficulty=? WHERE id=?",
      [$game_id,$title,$overview,$difficulty,$id],'isssi');
...
if($action==='edit'){
  ...
  if(!$w){ echo '<div class="card">Data tidak ditemukan</div>'; require __DIR__.'/_footer.php'; exit; }
  ...
  foreach($games as $g){ $sel = $g['id']==$w['game_id']?'selected':''; echo '<option value="'.$g['id'].'" '.$sel.'>'.e($g['title']).'</option>'; }
  echo '</select></label>';
  ...
  foreach(['Easy','Medium','Hard'] as $d){ $sel=$d===$w['difficulty']?'selected':''; echo '<option '.$sel.'>'.$d.'</option>'; }
  echo '</select></label>';
  ...
  csrf_field();
  echo '<label>Game<select name="game_id">';
  foreach($games as $g){ echo '<option value="'.$g['id'].'">'.e($g['title']).'</option>'; }
  echo '</select></label>';
...
$rows = db_all($mysqli,"SELECT w.id, w.title, w.difficulty, g.title AS game FROM walkthroughs w JOIN games g ON g.id=w.game_id ORDER BY w.id DESC");
