...
if($action==='create' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    db_exec($mysqli,"INSERT INTO chapters(walk_id,title,content,order_number) VALUES(?,?,?,?)",
      [$walk_id,$title,$content,$order_number],'issi');
...
if($action==='update' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    $order_number = (int)($_POST['order_number'] ?? 1);
    $content = str_trim($_POST['content'] ?? '');
    db_exec($mysqli,"UPDATE chapters SET walk_id=?, title=?, content=?, order_number=? WHERE id=?",
      [$walk_id,$title,$content,$order_number,$id],'issii');
...
if($action==='edit'){
  ...
  foreach($walks as $w){ $sel=$w['id']==$c['walk_id']?'selected':''; echo '<option value="'.$w['id'].'" '.$sel.'>'.e($w['title']).'</option>'; }
  echo '</select></label>';
  ...
  csrf_field();
  echo '<label>Walkthrough<select name="walk_id">';
  foreach($walks as $w){ echo '<option value="'.$w['id'].'">'.e($w['title']).'</option>'; }
  echo '</select></label>';
