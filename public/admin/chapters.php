...
function yt_id_from_url(?string $u): string {
  $u = trim((string)$u);
  if ($u === '') return '';
  if (preg_match('~(?:v=|youtu\.be/)([A-Za-z0-9_-]{6,})~', $u, $m)) return $m[1];
  return '';
}
...
function upload_chapter_image(string $seed): ?string {
  ...
  if ($_FILES['image']['error'] !== UPLOAD_ERR_OK) throw new Exception('Upload gambar gagal.');

  $tmp  = $_FILES['image']['tmp_name'];
  $size = (int)$_FILES['image']['size'];
  if ($size > 3*1024*1024) throw new Exception('Gambar > 3MB.');
...
.tbl colgroup col.c-no      { width:72px; }
.tbl colgroup col.c-walk    { width:26%;  }
.tbl colgroup col.c-title   { width:auto; }
.tbl colgroup col.c-prev    { width:128px;}
.tbl colgroup col.c-order   { width:82px; }
.tbl colgroup col.c-actions { width:190px;}
.tbl tbody tr{transition:.16s transform ease, .16s box-shadow ease}
...
.tbl tbody td:first-child{border-radius:14px 0 0 14px}
.tbl tbody td:last-child {border-radius:0 14px 14px 0}
...
.thumb{
  width:96px; height:60px; border-radius:10px; overflow:hidden;
  border:1px solid rgba(255,255,255,.12); background:#0f0f16; position:relative;
}
.thumb img{ width:100%; height:100%; object-fit:cover; display:block }
.thumb .badge-yt{
  position:absolute; right:6px; bottom:6px;
  padding:.15rem .45rem; font-size:.7rem; font-weight:800;
...
.chip-del:hover{ box-shadow:0 8px 22px rgba(239,68,68,.35) }
/* drag & drop area (form atas) */
.dz{position:relative;border:1px dashed rgba(167,139,250,.55);border-radius:14px;padding:12px;
    background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.015));cursor:pointer;text-align:center}
.dz.drag{border-color:#a78bfa;box-shadow:0 0 0 3px rgba(167,139,250,.25) inset}
.dz .thumb-preview{display:none;margin-top:10px;border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,.1)}
.dz img{display:block;max-width:100%}
.hidden{display:none}
.tip{border-left:3px solid var(--primary);padding:.6rem .8rem;margin:.2rem 0;background:rgba(167,139,250,.08);border-radius:10px;opacity:.95}
@media (max-width: 880px){
  .tbl colgroup col.c-actions{ width:160px }
...
if ($action === 'edit') {
  ...
  if (!$c) { echo '<div class="card">Data tidak ditemukan</div>'; require __DIR__ . '/_footer.php'; exit; }
?>
  <div class="card">
    <h2>Edit</h2>
    <form method="post" enctype="multipart/form-data" class="grid">
      ...
      <label>YouTube URL (opsional)
        <input class="input" name="youtube_url" placeholder="https://www.youtube.com/watch?v=…" value="<?= e($c['youtube_url'] ?? '') ?>">
      </label>
      <?php endif; ?>
...
<div class="card">
  ...
  <table class="tbl">
    <colgroup>
      <col class="c-no"><col class="c-walk"><col class="c-title">
      <col class="c-prev"><col class="c-order"><col class="c-actions">
    </colgroup>
    <thead>
      <tr>
        <th>No</th>
    ...
    <tbody>
      <?php if (!$rows): ?>
        <tr><td colspan="6" style="text-align:center;opacity:.7;padding:24px">Belum ada chapter.</td></tr>
      <?php endif; ?>

      <?php $no=1; foreach ($rows as $r): $yt = yt_id_from_url($r['youtube_url'] ?? ''); ?>
        <tr>
          <td><span class="badge"><?= $no++ ?></span></td>
          <td><div class="cell"><span class="text-clip" title="<?= e($r['walk']) ?>"><?= e($r['walk']) ?></span></div></td>
          <td><div class="cell"><span class="text-clip" title="<?= e($r['title']) ?>"><?= e($r['title']) ?></span></div></td>
          <td>
            <div class="thumb" title="<?= $yt ? 'YouTube' : (empty($r['image_url']) ? 'Tidak ada preview' : 'Gambar') ?>">
              ...
              <?php else: ?>
                <img src="data:image/svg+xml;charset=utf-8,<?= rawurlencode('<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'192\' height=\'120\'><rect width=\'100%\' height=\'100%\' fill=\'#0f0f16\'/><text x=\'50%\' y=\'54%\' fill=\'#888\' font-size=\'12\' font-family=\'Arial\' text-anchor=\'middle\'>no preview</text></svg>') ?>">
              <?php endif; ?>
            </div>
          </td>
          ...
          <td>
            <div class="actions">
              ...
              <a class="chip-btn chip-del"
                 href="chapters.php?action=delete&id=<?= (int)$r['id'] ?>"
