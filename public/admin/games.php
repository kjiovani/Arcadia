...
if($action==='create' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    db_exec($mysqli,"INSERT INTO games(title,genre,platform,release_year,image_url,description) VALUES(?,?,?,?,?,?)",
      [$title,$genre,$platform,$release_year,$image_url,$description],'sssiss');
...
if($action==='update' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    $image_url = str_trim($_POST['image_url'] ?? '');
    $description = str_trim($_POST['description'] ?? '');
    db_exec($mysqli,"UPDATE games SET title=?,genre=?,platform=?,release_year=?,image_url=?,description=? WHERE id=?",
      [$title,$genre,$platform,$release_year,$image_url,$description,$id],'sssissi');
