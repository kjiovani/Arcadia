...
function handle_cover_upload(string $title): ?string {
  if (empty($_FILES['cover']) || $_FILES['cover']['error'] === UPLOAD_ERR_NO_FILE) return null;
  ...
  if ($size > 2 * 1024 * 1024) throw new Exception('Cover terlalu besar (>2MB).');
...
if ($action === 'edit') {
  ...
  <div class="card">
    ...
    <form method="post" action="games.php" class="grid" enctype="multipart/form-data">
      ...
      <label>Tahun Rilis <input class="input" type="number" name="release_year" value="<?= e($g['release_year']) ?>"></label>
          ...
          <td class="act">
            <div class="actions">
              ...
              <a class="chip-btn chip-del"
                 href="games.php?action=delete&id=<?= (int)$r['id'] ?>"
