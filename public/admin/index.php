...
.page-head h1{margin:0;font-weight:800;letter-spacing:.2px}
.page-sub{opacity:.8;margin-top:4px}
...
.kpi:before{
  content:"";position:absolute;inset:-40% -20% auto auto;height:160px;width:160px;border-radius:50%;
  background:radial-gradient(circle at 30% 30%, var(--primary), transparent 60%);
  opacity:.26;filter:blur(8px);
}
.kpi .label{opacity:.85;font-weight:700}
.kpi .value{font-size:40px;line-height:1.1;margin-top:6px;font-weight:900;letter-spacing:.5px}
.kpi .badge{
  margin-top:10px;display:inline-block;padding:8px 14px;border-radius:12px;font-weight:700;
  background:rgba(167,139,250,.22);border:1px solid rgba(167,139,250,.45);text-decoration:none;color:inherit
}
...
.panel{
  border-radius:16px;padding:16px;background:var(--panel);border:1px solid var(--border);
...
.item{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:12px;border:1px solid var(--border);background:rgba(255,255,255,.02)}
.item:hover{border-color:var(--primary)}
.item .title{font-weight:700}
.item .meta{margin-left:auto;opacity:.75;font-size:.9rem}
.actions{display:flex;gap:10px;flex-wrap:wrap}
.actions .btn{padding:10px 14px;border-radius:12px;border:1px solid var(--border);text-decoration:none;color:inherit}
.actions .btn:hover{border-color:var(--primary);box-shadow:0 8px 22px var(--ring)}
</style>
