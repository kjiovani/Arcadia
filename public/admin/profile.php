...
if (!$u) {
  $u = ['id'=>$user_id,'name'=>'','email'=>'','role'=>'OWNER','avatar_url'=>'','banner_url'=>'','bio'=>''];
}
  ...
  try {
    ...
    if ($HAS_PWHASH) {
      ...
      if ($newpass !== '') {
        ...
        $fields[] = 'password_hash=?'; $bind[] = password_hash($newpass, PASSWORD_DEFAULT);
      }
    }
...
/* ---------- Stats ---------- */
function dir_size_mb($path){ if(!is_dir($path))return 0; $s=0;$it=new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path,FilesystemIterator::SKIP_DOTS)); foreach($it as $f){ if($f->isFile())$s+=$f->getSize(); } return round($s/1024/1024,2);}
...
.pro-name{font-size:1.6rem;font-weight:900;margin:.25rem 0 0}
...
@media(min-width:980px){ .edit-wrap{ grid-template-columns: 1.08fr .92fr; } }
...
/* PANEL KANAN — compact & rapi */
.media-pane{max-width:440px;margin-left:auto}
...
/* Avatar kecil & center */
.frame.avatar{
  width:148px;height:148px;border-radius:22px;overflow:hidden;border:1px solid var(--border);
  background:#111;display:block;margin:8px auto 8px; object-fit:cover
}
#avatarPreview{
  width:136px;height:136px;border-radius:18px;border:1px solid rgba(255,255,255,.1);
  display:none;margin:6px auto 6px;object-fit:cover
}

/* Banner proporsional (fix anti-gepeng) */
.frame.banner{
  width:100%; height:210px; object-fit:cover; border-radius:18px;
  overflow:hidden; border:1px solid var(--border); background:#111; display:block
}
#bannerPreview{
  width:100%; height:210px; object-fit:cover; display:none; border-radius:12px;
  border:1px solid rgba(255,255,255,.1); margin:8px 0 6px
}
@media (max-width:520px){ .frame.banner,#bannerPreview{ height:180px; } }
@media (min-width:1200px){ .frame.banner,#bannerPreview{ height:240px; } }
    ...
    <div class="card">
      ...
      <div class="grid-3">
        <div class="stat card" style="box-shadow:none">
          <div class="help" style="margin:0">Total Konten</div>
          <div style="font-size:1.4rem;font-weight:900"><?= number_format($stat_games + $stat_walks + $stat_chapters) ?></div>
        </div>
        <div class="stat card" style="box-shadow:none">
          <div class="help" style="margin:0">Uploads Terpakai</div>
          <div style="font-size:1.4rem;font-weight:900"><?= $uploads_used_mb ?> MB</div>
          <div class="help" style="margin-top:6px">Batas upload: <?= $upload_limit_mb ?> MB • POST: <?= $post_limit_mb ?> MB</div>
        </div>
        <div class="stat card" style="box-shadow:none">
          <div class="help" style="margin:0">Alamat IP</div>
          <div style="font-size:1.25rem"><?= e($ip_addr) ?></div>
          <div class="help" style="margin-top:6px">Akses: <?= e($loginTime) ?></div>
        </div>
      </div>
    </div>
            ...
            <?php if ($HAS_AVATAR): ?>
              <div class="tile">
                <div class="tile-head">
                  ...
                  <img class="frame avatar" src="<?= e($avatarSrc) ?>" alt="Avatar">
                </div>
                <div>
            ...
            <?php if ($HAS_BANNER): ?>
              <div class="tile">
                <div class="tile-head">
                  ...
                  <?php if ($bannerSrc): ?>
                    <img class="frame banner" src="<?= e($bannerSrc) ?>" alt="Banner">
                  <?php else: ?>
