...
if($action==='create' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    flash('ok','Tag dibuat.'); header('Location: tags.php'); exit;
...
if($action==='update' && $_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    flash('ok','Tag diperbarui.'); header('Location: tags.php'); exit;
  } catch(Exception $e){ flash('err',$e->getMessage()); }
}
if($action==='delete'){
  $id = (int)($_GET['id'] ?? 0);
  db_exec($mysqli,"DELETE FROM tags WHERE id=?",[$id],'i');
  flash('ok','Tag dihapus.'); header('Location: tags.php'); exit;
}
