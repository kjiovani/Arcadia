...
if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if (mysqli_affected_rows($mysqli) === 0) {
      ...
      if ($exists) throw new Exception('Tag sudah ada.');
    }
...
if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($dupe) throw new Exception('Nama tag sudah dipakai.');
...
.tag-chip .dot{width:8px;height:8px;border-radius:999px;background:#a78bfa;box-shadow:0 0 0 3px rgba(167,139,250,.18)}
.card h2 .hint{font-size:.9rem;opacity:.7;font-weight:600;margin-left:.5rem}

.tbl{
  width:100%;border-collapse:separate;border-spacing:0 12px;table-layout:fixed;
}
.tbl colgroup col.c-no{width:72px}
.tbl colgroup col.c-name{width:auto}
.tbl colgroup col.c-used{width:110px}
.tbl colgroup col.c-act{width:190px}
.tbl thead th{
  position:sticky;top:0;z-index:5;padding:14px 16px;text-align:left;
  font-weight:800;background:rgba(17,17,24,.7);
  backdrop-filter:saturate(160%) blur(6px);border-bottom:1px solid rgba(255,255,255,.08)
}
...
.badge{
  display:inline-block;min-width:28px;text-align:center;padding:.26rem .6rem;
  border-radius:999px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.06);font-weight:800
}
.actions{display:flex;gap:10px;justify-content:flex-end}
.chip-btn{
  display:inline-flex;align-items:center;gap:8px;
  padding:8px 14px;border-radius:999px;font-weight:800;color:#eee;text-decoration:none;
...
<div class="card">
  ...
  <div class="tag-cloud">
    <?php foreach ($all_for_cloud as $t): ?>
      <span class="tag-chip"><span class="dot"></span><?= e($t['name']) ?><?php if ($t['used_count']): ?> · <?= (int)$t['used_count'] ?><?php endif; ?></span>
    <?php endforeach; ?>
  </div>
</div>
<?php endif; ?>
...
if ($action === 'edit') {
  ...
  if (!$t) { echo '<div class="card">Data tidak ditemukan</div>'; require __DIR__.'/_footer.php'; exit; }
?>
  <div class="card">
    <h2>Edit Tag</h2>
    <form method="post" class="grid">
      ...
      <div class="tip">Gunakan nama singkat & mudah dicari (contoh: <em>Boss</em>, <em>Build</em>, <em>Early Game</em>).</div>
...
<div class="card">
  ...
  <table class="tbl">
    <colgroup>
      <col class="c-no"><col class="c-name"><col class="c-used"><col class="c-act">
    </colgroup>
    <thead>
      <tr>
        <th>No</th>
    ...
    <tbody>
      <?php if (!$rows): ?>
        <tr><td colspan="4" style="text-align:center;opacity:.7;padding:24px">Belum ada tag.</td></tr>
      <?php endif; ?>
