...
function ext_from_mime($mime){ return ['image/png'=>'png','image/jpeg'=>'jpg','image/webp'=>'webp'][$mime]??null; }
function upload_image_local($file,$subdir='branding',$seed='img'){
  ...
  $ext = ext_from_mime($mime); if(!$ext) throw new Exception('Format harus JPG/PNG/WEBP.');
  ...
  if(!move_uploaded_file($file['tmp_name'],$dir.'/'.$name)) throw new Exception('Gagal menyimpan file.');
...
try{
  if($act==='set_logo'){
    if(empty($_FILES['logo'])) throw new Exception('File tidak ada.');
  ...
  if($act==='set_colors'){
    ...
    set_setting('brand_color_p1',$p1); set_setting('brand_color_p2',$p2); set_setting('brand_color_p3',$p3);
  ...
  if($act==='set_favicon'){
    if(empty($_FILES['ico'])) throw new Exception('File tidak ada.');
