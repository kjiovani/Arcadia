...
foreach (array_merge($suggWalks, $suggGames) as $r) {
  ...
  if ($t !== '' && !isset($pool[mb_strtolower($t)])) $pool[mb_strtolower($t)] = $t;
}
...
<style>
  ...
  .search-sub{ margin:.25rem 0 1rem; opacity:.9 }
  ...
  .hotchips{ display:flex; flex-wrap:wrap; gap:8px; margin-top:14px }
  .chip{
  ...
  .chip .tag{opacity:.8;font-size:.85rem}
  ...
  .res-card{
    ...
    border-radius:14px; background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.015));
  }
  @media (max-width:680px){ .res-card{ grid-template-columns:68px 1fr } .res-go{ grid-column:1 / -1; justify-self:flex-end } }

  .res-thumb{
    width:84px;height:84px;border-radius:10px;overflow:hidden;background:rgba(255,255,255,.04);
    display:grid;place-items:center;border:1px solid rgba(255,255,255,.06);
  }
  .res-thumb img{ width:100%;height:100%;object-fit:cover }
  .res-fallback{ font-weight:800;font-size:1.2rem;opacity:.9 }

  .res-title{ display:flex; align-items:center; gap:8px; flex-wrap:wrap; margin-bottom:4px }
  .res-title a{ color:inherit; text-decoration:none; font-weight:800 }
  .res-title a:hover{ text-decoration:underline }
  .res-meta{ opacity:.9 }
  .res-excerpt{ margin-top:6px; opacity:.95; line-height:1.6 }

  .res-go .btn.ghost{
    background:transparent;border:1px solid rgba(167,139,250,.35);color:#e9e9ff;border-radius:12px;padding:.55rem 1rem;
  ...
  .res-go .btn.ghost:focus-visible{
    background: var(--primary) !important; border-color: var(--primary) !important; color:#0f0f16;
    box-shadow:0 8px 22px var(--ring); transform: translateY(-1px);
  }

  /* highlight kecil */
  .hl{ background: linear-gradient(0deg, rgba(167,139,250,.45), rgba(167,139,250,.45)); color:#0f0f16; padding:0 .15rem; border-radius:.25rem }
</style>
...
<div class="search-hero card">
  ...
  <form method="get" class="search-row" action="search.php">
    <input class="input" type="text" name="q" value="<?= e($q) ?>" placeholder="Judul game atau walkthrough…" list="q_suggest" autofocus>
    <button class="btn">Cari</button>
  </form>

  <?php if (!empty($hot)): ?>
    <div class="hotchips" aria-label="Sedang dicari">
      <?php foreach ($hot as $i => $h): $kw = trim($h['keyword']); if ($kw==='') continue; ?>
        <a class="chip" href="/arcadia/public/search.php?q=<?= urlencode($kw) ?>"><?= e($kw) ?> <span class="tag">· <?= (int)$h['n'] ?></span></a>
      <?php endforeach; ?>
  ...
  <?php if ($q !== ''): ?>
    ...
    <?php if (!$rows): ?>
      <div class="card" style="padding:18px;border-radius:14px">
        Tidak ada hasil. Coba istilah lain atau telusuri <a href="/arcadia/public/games.php" style="color:inherit;text-decoration:underline">daftar game</a>.
      </div>
    <?php else: ?>
