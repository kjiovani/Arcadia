...
$sqlGames = "SELECT id,title,genre,platform,image_url,
             ...
             FROM games";
if ($activeGenre !== '') { $sqlGames .= " WHERE genre=?"; $params[] = $activeGenre; $types .= 's'; }
...
foreach (array_merge($suggWalks, $suggGames) as $r) {
  ...
  if ($t !== '' && !isset($_tmpS[mb_strtolower($t)])) $_tmpS[mb_strtolower($t)] = $t;
}
...
function diff_cls($d){ $d=strtolower(trim($d)); return $d==='easy'?'easy':($d==='hard'?'hard':'medium'); }
...
<style>
  ...
  .upd-item{display:grid;grid-template-columns:120px 1fr;gap:12px;padding:12px;border-radius:14px;background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.015));border:1px solid rgba(255,255,255,.06)}
  ...
  .clamp-2{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;line-height:1.6}
  .btn.ghost.btn-open{background:transparent;border:1px solid rgba(167,139,250,.35);color:#e9e9ff;border-radius:12px;padding:.55rem 1rem;transition:background .18s,border-color .18s,box-shadow .18s,transform .14s}
  .btn.ghost.btn-open:hover,.btn.ghost.btn-open:focus-visible{background:var(--primary)!important;border-color:var(--primary)!important;color:#0f0f16;box-shadow:0 8px 22px var(--ring);transform:translateY(-1px)}
  .btn.ghost.btn-open:active{transform:translateY(0);box-shadow:0 4px 12px var(--ring)}
  .cover-frame{position:relative;border-radius:12px;overflow:hidden}
  .cover-frame.editing{outline:2px dashed rgba(167,139,250,.55);outline-offset:4px}

  /* logo kecil di judul section */
  .title-logo{height:22px;width:auto;vertical-align:-4px;margin-right:8px;border-radius:6px;border:1px solid rgba(255,255,255,.08)}
</style>
...
<div class="hero">
  ...
  <datalist id="q_suggest">
    <?php foreach ($suggestions as $s): ?><option value="<?= e($s) ?>"></option><?php endforeach; ?>
  </datalist>
...
<!-- (2) Daftar Game — 4 item -->
<section id="games" class="section card">
  <h1><?php if($logo_games): ?><img class="title-logo" src="<?= e($logo_games) ?>" alt=""><?php endif; ?>Daftar Game</h1>
  <p class="small">Pilih game untuk melihat walkthrough.</p>

  <?php $gamesLimited=array_slice($games,0,4); $needFill=max(0,4-count($gamesLimited)); ?>
  <div class="games-grid">
      ...
      <div class="game-item v2">
        <div class="cover-frame">
          <?php if (!empty($g['image_url'])): ?>
            ...
            <img class="cover-adjustable" data-table="games" data-id="<?= (int)$g['id'] ?>"
                 src="<?= e($g['image_url']) ?>" alt="<?= e($g['title']) ?>"
        ...
        <div class="game-meta">
          <div class="title-row"><h3><?= e($g['title']) ?></h3></div>
          <div class="meta-row small"><?= e($g['platform']) ?></div>
          <p class="desc clamp-2"><?= e($g['excerpt']) ?>…</p>
        </div>

        <?php $detailUrl='/arcadia/public/game.php?id='.$g['id']; if(!is_user_logged_in()){ $detailUrl='/arcadia/public/auth/login.php?next='.urlencode($detailUrl);} ?>
    ...
    <?php for($i=0;$i<$needFill;$i++): ?>
      <div class="game-item v2 placeholder">
        ...
        <div class="game-meta">
          <div class="title-row"><h3>Tambahkan Game</h3></div>
  ...
  <div class="more-wrap"><a class="btn btn-pill btn-more-logout" href="/arcadia/public/games.php"><span>Lainnya</span></a></div>
</section>

<!-- (3) Panduan Unggulan -->
<section class="section card">
  <h2 style="margin-bottom:.25rem"><?php if($logo_feat): ?><img class="title-logo" src="<?= e($logo_feat) ?>" alt=""><?php endif; ?>Panduan Unggulan</h2>
  ...
  <?php else: ?>
    <div id="feat4" class="feat4-wrap">
      <div class="feat4-viewport"><div class="feat4-track">
        <?php foreach ($feat4 as $f): $initial=mb_strtoupper(mb_substr($f['game'],0,1)); ?>
          <article class="feat4-card">
            <div class="feat4-thumb cover-frame">
              <?php if (!empty($f['game_image'])): ?>
                <?php $fx=(int)($f['g_fx'] ?? 50); $fy=(int)($f['g_fy'] ?? 50); ?>
                <img class="cover-adjustable" data-table="games" data-id="<?= (int)$f['game_id'] ?>"
                     src="<?= e($f['game_image']) ?>" alt="<?= e($f['game']) ?>"
                     style="width:100%;height:100%;object-fit:cover;object-position:<?= $fx ?>% <?= $fy ?>%">
              <?php else: ?><div class="feat4-fallback"><?= e($initial) ?></div><?php endif; ?>
            </div>
            ...
            <div class="feat4-body">
              ...
              <div class="feat4-meta">🎮 <a href="game.php?id=<?= (int)$f['game_id'] ?>" class="small" style="color:inherit;text-decoration:none"><?= e($f['game']) ?></a></div>
              <p class="feat4-desc clamp-2"><?= e(mb_strimwidth($f['excerpt'] ?? '',0,160,'…','UTF-8')) ?></p>
              <?php $openUrl='/arcadia/public/walkthrough.php?id='.(int)$f['id']; if(!is_user_logged_in()){ $openUrl='/arcadia/public/auth/login.php?next='.urlencode($openUrl);} ?>
...
<!-- (4) Baru Diupdate ✨ -->
<section class="section card">
  <h2 style="margin-bottom:.25rem"><?php if($logo_recent): ?><img class="title-logo" src="<?= e($logo_recent) ?>" alt=""><?php endif; ?>Baru Diupdate ✨</h2>
  ...
  <?php else: ?>
    <div class="grid-upd">
      <?php foreach ($recentWalks as $rw):
        ...
        if(!is_user_logged_in()){ $openUrl='/arcadia/public/auth/login.php?next='.urlencode($openUrl); }
      ...
      <article class="upd-item">
        <div class="upd-thumb cover-frame">
          ...
          <?php else: ?><div class="upd-fallback"><?= e($initial) ?></div><?php endif; ?>
        </div>
        <div>
          ...
          <?php endif; ?>
          <div class="small" style="margin-top:8px;display:flex;align-items:center;justify-content:space-between;gap:10px">
...
<script>
  ...
  // Mini Editor Cover (aktif hanya untuk OWNER)
  <?php
    $me = function_exists('current_user') ? current_user() : ($_SESSION['user'] ?? null);
    $isOwner = $me && strtoupper($me['role'] ?? '') === 'OWNER';
    echo 'const __IS_OWNER__ = '.($isOwner?'true':'false').';';
  ?>

  if (__IS_OWNER__) {
    const imgs = document.querySelectorAll('img.cover-adjustable');
    imgs.forEach(img => {
      img.style.cursor = 'grab';
      const frame = img.closest('.cover-frame'); if (frame) frame.classList.add('editing');
      let down=false;
      img.addEventListener('pointerdown',e=>{down=true;img.setPointerCapture(e.pointerId);img.style.cursor='grabbing';});
      img.addEventListener('pointerup',e=>{down=false;img.style.cursor='grab';});
      ...
      img.addEventListener('dblclick', async ()=>{
        ...
        const fx = parseInt(px); const fy = parseInt(py);
        const table = img.dataset.table; const id = img.dataset.id;
        try{
        ...
        }catch{ alert('Gagal menyimpan fokus cover.'); }
      });
    });
  }
</script>
