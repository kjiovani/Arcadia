...
{
  if (strpos($web, '/arcadia/public/') === 0)
    return __DIR__ . '/' . ltrim(substr($web, strlen('/arcadia/public/')), '/');
  if (strpos($web, '/arcadia/') === 0)
    return dirname(__DIR__) . '/' . ltrim(substr($web, strlen('/arcadia/')), '/');
  return null;
}
function normalize_logo_url(string $u): string
{
  ...
  if (preg_match('~^https?://~i', $u))
    return $u; // URL absolut
  $u = preg_replace('~//+~', '/', $u);
  if (preg_match('~^/?uploads/~', $u))
    $u = '/arcadia/public/' . ltrim($u, '/');
...
<head>
  ...
  <style>
    ...
    :root {
      --brandLogoH: clamp(34px, 2.2vw + 22px, 50px);
    }

    /* atur max jika ingin lebih besar */
