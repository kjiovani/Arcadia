...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if (mb_strlen($pass) < 6) throw new Exception('Password minimal 6 karakter.');
...
<div class="container card" style="max-width:560px">
  <h1>Daftar Akun</h1>
  <?php if(!empty($err)): ?><div class="alert"><?= e($err) ?></div><?php endif; ?>
  <form method="post" class="grid">
    ...
    <div class="small">Sudah punya akun? <a href="/arcadia/public/auth/login.php<?= $next?('?next='.urlencode($next)) : '' ?>">Masuk</a></div>
  </form>
</div>
