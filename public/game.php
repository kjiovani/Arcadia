...
if(!$game){ echo '<div class="card">Game tidak ditemukan.</div>'; include __DIR__.'/_footer.php'; exit; }

echo '<div class="card">';
echo '<h1>'.e($game['title']).'</h1>';
echo '<div class="small">'.e($game['genre']).' • '.e($game['platform']).' • Rilis '.e($game['release_year']).'</div>';
if(!empty($game['image_url'])) echo '<img src="'.e($game['image_url']).'" alt="" style="width:100%;max-height:300px;object-fit:cover;border-radius:12px;margin:.75rem 0" />';
...
if(!$walks){ echo '<p class="small">Belum ada walkthrough.</p>'; }
foreach($walks as $w){
